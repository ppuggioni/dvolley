...
from dvolley.config import DATE_FORMATS, TEAM_NAME_TO_FIX

logger = logging.getLogger(__name__)


def list_files_sorted(dir_path: str) -> list[str]:
...
def add_rally_metadata(
    ...
    for i, row in df.iterrows():
        ...
        if row[skill_col] == "Serve":
            serve_team = (
                row[team_col]
                if pd.notna(row[team_col]) and row[team_col] != ""
                else None
            )
            if serve_team:
...
def concat_align_and_save(dfs: list[pd.DataFrame], output_path: str) -> pd.DataFrame:
    ...
    if len(all_affected_ids) > 1:
        ...
        logger.warning("MANUAL FIX: Team '%s' has multiple IDs: %s", TEAM_NAME_TO_FIX, all_affected_ids)
        logger.warning("Consolidating all to canonical ID: %s", canonical_id)
        ...
        for old_id in all_affected_ids:
            if old_id != canonical_id:
                ...
                logger.info("Replaced team_id %s -> %s", old_id, canonical_id)
