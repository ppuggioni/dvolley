...
def _render_summary_tables(result: SideoutTouchResult):
    ...
    with c1:
        ...
        styler = _build_table_styler(
            ...
            baseline_by_col={
                "% points WITH opp serve errors": (
                    lambda raw_df, _: raw_df.at["Total", "% points WITH opp serve errors"]
                    if "Total" in raw_df.index
                    else None
                ),
                "% points WITHOUT opp serve errors": (
                    lambda raw_df, _: raw_df.at["Total", "% points WITHOUT opp serve errors"]
                    if "Total" in raw_df.index
                    else None
                ),
            },
    ...
    with c2:
        ...
        raw = result.attacker_summary.copy()
        display = _prepare_table(raw)
        baseline = (
            (raw["Points"].sum() / raw["Attacks"].sum())
            if not raw.empty and raw["Attacks"].sum() > 0
            else None
        )
        styler = _build_table_styler(
            display,
            raw,
            colored_cols=["% points on attacks"],
            baseline_by_col={"% points on attacks": baseline},
            skip_rows=set(),
        )
        st.dataframe(styler, use_container_width=True)
    ...
    with c3:
        ...
        styler = _build_table_styler(
            ...
            baseline_by_col={
                "% points on actions": (
                    lambda raw_df, _: raw_df.at["Total", "% points on actions"]
                    if "Total" in raw_df.index
                    else None
                ),
                "% of total actions": equal_share,
    ...
    for i, class_name in enumerate(class_names):
        with cols[i % 3]:
            ...
            styler = _build_table_styler(
                ...
                baseline_by_col={
                    "%": (
                        lambda raw_df, _: raw_df.at["Total", "%"] if "Total" in raw_df.index else None
                    )
                },
                skip_rows={"Total"},
            )
